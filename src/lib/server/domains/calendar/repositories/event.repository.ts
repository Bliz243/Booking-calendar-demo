...
import { db, generateId } from '$lib/server/db/drizzle';
import {
	events,
	eventExceptions,
	type Event,
...
export const eventRepository = {
	 ...
	 */
	deleteByCalendarId(calendarId: string): void {
		db.delete(events).where(eq(events.calendarId, calendarId)).run();
	},
	 ...
	 */
	deleteExceptionsByEventId(eventId: string): void {
		db.delete(eventExceptions).where(eq(eventExceptions.eventId, eventId)).run();
	}
};
